...
namespace LS
{
	...
	ThreadParameter m_threadParameters[Engine::NUM_CONTEXT];
	...
	private:
		...
		std::thread												m_defaultDrawThread;
		std::thread												m_textureDrawThread;
		// Blah stuff
		bool firstRun = true;
	...
	public:
		...
		void Draw(D3D12_VERTEX_BUFFER_VIEW& bufferView, uint64_t vertices, std::optional<uint64_t> instances = 1u)
		{
			...
			m_pCurrFrameContext->CommandList[m_frameResourceIndex]->DrawInstanced(vertices, instances.value(), 0, 0);
		}
		...
		void MoveToNextFrame()
		{
			...
			m_frameResourceIndex = (m_frameResourceIndex + 1) % Engine::FRAME_COUNT;
		...
		void CreateThreads()
		{
			...
			{
				while (threadIndex < Engine::NUM_CONTEXT)
				{
					// Prepare allocators
			...
			m_defaultDrawThread = std::thread(work, 1);
